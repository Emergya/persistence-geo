...
public class FolderStyleDecorator implements IFolderDecorator {
    ...
    @Override
    public void applyStyle(FolderDto folder, List tree, String parent, FolderStyle style, Integer level) {
        if (folder != null) {
            if (FolderStyle.NORMAL.equals(style)) {
                if (folder.getFolderList() != null && folder.getFolderList().size() > 0) {
