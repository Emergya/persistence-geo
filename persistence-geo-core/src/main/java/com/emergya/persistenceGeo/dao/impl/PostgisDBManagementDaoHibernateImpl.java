 ...
 */
@Repository
public class PostgisDBManagementDaoHibernateImpl extends HibernateDaoSupport
		implements DBManagementDao {
	...
	public long getTableSize(String table_name) {
		...
		Long result;
		try {
			result = ((BigInteger) getSession().createSQLQuery(sql)
	...
	public GeometryType getTableGeometryType(String tableName) {
		...
		if (StringUtils.contains(dbType, "POINT")) {
			...
			throw new IllegalStateException(String.format(
					"Type %s is not a PersistenceGeo supported GeometryType.",
					dbType));
		}
	}
		...
		getSession().doWork(new Work() {
			...
			public void execute(Connection connection) throws SQLException {
				...
				if (pksRS.first()) {
					...
					String createPkSql = MessageFormat.format(ADD_TABLE_PK,
							schema, destinationTable, primaryKey);
	...
	private boolean createLayerTable(String tableName, List<DynaBean> columns,
		...
		try {
			...
			logger.error(String.format(
					"Error creando la tabla \"%s\" de tipo %s", tableName,
					GEOMETRY_TYPE_SQL), e);
			created = false;
		}
